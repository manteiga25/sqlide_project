 ...
 //       final boolean isForeign = meta.isForeign;
    ...
    protected boolean dropSpecialColumn(String table, ArrayList<ColumnMetadata> columns) {
        ...
        try {
            ...
            if (!copyDataTable(table, columns)) {
                System.out.println("erro " + MsgException);
                throw new SQLException(MsgException);
            }
  ...
  /*  @Override
    ...
    private boolean createTableSpecial(final String table, final ArrayList<ColumnMetadata> columns) {
        ...
        StringBuilder command = new StringBuilder("CREATE TABLE " + table + " ("), primary = new StringBuilder();
      ...
      //      final String ForeignTable = column.ForeignKey == null || Objects.equals(column.ForeignKey[0], "") ? "REFERENCES " + column.ForeignKey[0] + "(" + column.ForeignKey[1] + ")" : "";
         //   final String[] ForeignTable = column.ForeignKey;
           ...
           //     ColumnName = column.Name + " " + Type + " " + NotNull + " PRIMARY KEY";
                ...
                primary.append(ColumnName).append(", ");
                System.out.println("primary " + ColumnName);
               // prime = " PRIMARY KEY ";
            }
            else if (isForeign) {
         ...
         //   command.append(ColumnName).append(" ").append(Type).append(NotNull).append(prime).append(IsUnique).append(Default).append(", ");
            command.append(ColumnName).append(" ").append(Type).append(NotNull).append(IsUnique).append(Default).append(", ");
        }

        if (!primary.isEmpty()) primary = new StringBuilder("PRIMARY KEY (" + primary.delete(primary.length()-2, primary.length()) + ")");

        command.append(primary).append(");");
   ...
   /* private boolean createTablePrime(final String table, final ArrayList<ColumnMetadata> columns) {
    ...
    public boolean deleteColumn(ArrayList<ColumnMetadata> columns, String columnName, String table) {
        final ColumnMetadata column = columns.stream().filter(c->c.Name.equals(columnName)).findFirst().get();
        if (column.foreign.isForeign || column.IsPrimaryKey) {
            final ArrayList<ColumnMetadata> tempMetadata = (ArrayList<ColumnMetadata>) columns.clone();
            tempMetadata.remove(column);
            return dropSpecialColumn(table, tempMetadata);
        } else {
            try {
  ...
  /*  private boolean deleteSpecialColumn(final ArrayList<ColumnMetadata> columns, final String table) {
        StringBuilder command = new StringBuilder("DROP TABLE " + table + " (");
